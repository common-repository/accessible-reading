...
class Utilities
{
     ...
     * @param array|false $args Optional. Arguments to pass to the hook's callback function. If false, then it will look for the next upcoming event for this hook, regardless of arguments.
     * @param bool $return_all Optional. If true and the cron job was found, this function will return an associative array with keys for `hook`, `timestamp`, and `args`
     * @return int|array|false The Unix timestamp of the next time the scheduled event will occur or false if not found.
     */
    public static function next_scheduled($hook, $args = false, $return_all = false)
    {
        if (is_array($args)) {
            return wp_next_scheduled($hook, $args); //Default WordPress functionality
        }
        //Not ideal...
        $crons = _get_cron_array();
        if (is_array($crons)) {
            foreach ($crons as $timestamp => $cron) {
                if (self::array_has_key($hook, $cron)) {
                    if ($return_all) {
                        self::debug_log($cron);
                        $key = array_key_first($cron['hook']);
                        return array(
                            'hook' => $hook,
                            'timestamp' => $timestamp,
                            'args' => $cron[$hook][$key]['args']
                        );
                    }
                    return $timestamp;
                }
    ...
    public static function discover_shortcodes($content, $tag, $closing_tag = false)
    {
        ...
        $normalized_content = self::normalize_html($content); //Remove all the junk
        $content = $normalized_content;

        //Search for shortcodes with attributes
        $start_tag = '[' . $tag . ' '; //Opens the start tag, assumes there are attributes because of the space
        $start_tag_end = ']'; //Closes the start tag
        if ($closing_tag) {
            //If this is NOT a self-closing tag, it will have a closing tag after content
            $closing_tag = '[/' . $tag . ']';
        }
        $start = strpos($content, $start_tag); //Get the first position of the opening tag
        if ($start !== false) {
            while ($start !== false) {
                $shortcode = array(
                    'atts' => array(),
                ...
                // [example att1="foo" att2="bar"]content[/example]

                //Find the closing start tag, offset by the end of this specific opening tag
                $start_tag_end_pos = strpos($content, $start_tag_end, $start);
                if ($start_tag_end_pos !== false) {
                    $atts_str_len = ($start_tag_end_pos + strlen($start_tag_end)) - $start; //String length, including closing tag

                    //Chop only the attributes e.g. `att1="foo" att2="bar"`
                    $atts_str =  trim(str_replace(
                        array($start_tag, $start_tag_end), //Remove the tag strings
                        '', //Replace with nothing
                        substr($content, $start,  $atts_str_len)
                    ));
                    ...
                    //Now check for content, starting where the start tag ended off
                    $content_end = $start + $atts_str_len; //End after the self-closing start tag
                    $closing_tag_pos = strpos($content,  $closing_tag, $content_end);
                    if ($closing_tag_pos !== false) {
                        $content_length = $closing_tag_pos - $content_end;
                        //Get the content between the opening and closing tags
                        $shortcode['content'] = substr($content, $content_end, $content_length);
                        $content_end += strlen($closing_tag); //End after the closing tag
                    }
                    ...
                    $content = substr($content, $content_end); //Remove this shortcode from the content to continue parsing in the while-loop
                    $start = strpos($content, $start_tag); //Get the position of the next opening tag
                }
            }
        }

        //Search for shortcodes without attributes
        $content = $normalized_content; //Reset the content back to it's original, normalized state
        ...
        if ($start !== false) {
            while ($start !== false) {
                ...
                $content_end = $start + strlen($start_tag); //End after the self-closing start tag
                if ($closing_tag) {
                    $closing_tag_pos = strpos($content, $closing_tag, $content_end);
                    if ($closing_tag_pos !== false) {
                        $content_length = $closing_tag_pos - $content_end;
                        $shortcode['content'] = substr($content, $content_end, $content_length); //Get the content between the opening and closing tag
                        $content_end += strlen($closing_tag); //End after the closing tag
                    }
                }
                //If anything was discovered in this shortcode, add it to the return object
     ...
     *
     * @param array $args. An associative array with keys allowed for `type` (`style` or `script` accepted),
     * `handle` (string), `url` (absolute URL to the resource), `dependencies` (array), `version` (string|false),
     * `media` (string media type, default is `all`), `inline_styles` (string), in_footer (bool, default is true),
     * and `localized_data` (array)
     * @return string The status (if debugging is enabled)
     */
    public static function optionally_load_resource($args)
    {
        $args = array_merge(array(
            'type' => '',
            'handle' => '', //The resource handle
            'url' => '', //An absolute URL to the resource to load. If excluded, can only enqueue previously registered resources.
            'dependencies' => array(), //An array of handles that the resource depends on
            'version' => false, //The version, if any. Default is `false` to exclude a version.
            'media' => 'all', //For stylesheets, this parameter is expecting a media type. Default is `all`
            'inline_styles' => '', //For stylesheets, add inline styles to a stylesheet when enqueued
            'in_footer' => true, //For scripts, this parameter is expecting a boolean value for whether to place script in the footer. Default is true.
            'localized_data' => array() //For scripts, additional data to localize a script
        ), $args);
        extract($args); // Import variables into the current symbol table from an array
        $result = '';
        if ($type == 'style') {
            ...
            if (wp_script_is($handle, 'registered') && !wp_script_is($handle, 'queue')) {
                ...
                if ($url) {
                    wp_enqueue_script($handle, $url, $dependencies, $version, $in_footer); //Place in footer
                    if (WP_DEBUG) {
    ...
    public static function debug_log($obj, $title = '')
    {
        if (WP_DEBUG && WP_DEBUG_LOG) {
            if ($title) {
                error_log('[ACCESSIBLE READING] ' . $title);
            }
            if (is_array($obj) || is_object($obj)) {
                error_log('[ACCESSIBLE READING] ' . print_r($obj, true));
            } else {
                error_log('[ACCESSIBLE READING] ' . $obj);
            }
        }
    }
     ...
     * Normalize HTML
     *
     * Sanitizes post content, removes HTML comments, and removes excess whitespace
     ...
     */
    public static function normalize_html($content = '')
    {
        if ($content) {
            return self::remove_excess_whitespace(wp_kses_post(self::remove_html_comments($content)));
        }
        return $content;
    }
     ...
     * @return string modified content
     */
    public static function remove_excess_whitespace($content = '')
    {
